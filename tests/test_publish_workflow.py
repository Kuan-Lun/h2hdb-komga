...
def _repository(tmp_path: Path) -> tuple[Path, str, str]:
    ...
    _write_version(repository, "0.8.1")
    version_commit = _commit(repository, "chore(release): bump version to 0.8.1")
...
def test_publish_version_rejects_a_decrease(tmp_path: Path) -> None:
    ...
    assert "project.version decreased from 0.8.1 to 0.7.0" in result.stderr
